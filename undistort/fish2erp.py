...
if __name__ == '__main__':
    ...
    parser.add_argument("--img_path", type=str, default='./samples/fe_l/*.png')
    parser.add_argument("--calib", type=str, default="./config/kitti360_leftfisheye.yaml")
    parser.add_argument("--rectified_path", type=str, default="kitti_demo")
    ...
    if not os.path.exists('./rectified/'+args.rectified_path): os.mkdir('./rectified/'+args.rectified_path)
    # Read YAML file
    with open(args.calib, 'r') as file:
    ...
    image_files = sorted(glob.glob(args.img_path))
    for i, image_path in enumerate(image_files):
        if i>=sample_num and sample_num>=0: break
        # get erp image
        ...
        K_new = np.array([[w / 2, 0, intrinsics['projection_parameters']['u0']],
                          [0, h / 2, intrinsics['projection_parameters']['v0']],
        ...
        cv2.imwrite(os.path.join('./rectified/'+args.rectified_path, image_path.split('/')[-1]), undistort_img)
        print('Written to:', os.path.join('./rectified/'+args.rectified_path, image_path.split('/')[-1]))
